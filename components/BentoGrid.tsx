...
import { PROJECTS, SOCIALS, PORTFOLIO_OWNER, PORTFOLIO_ROLE, PORTFOLIO_BIO } from '../constants';
...
const BentoGrid: React.FC<BentoGridProps> = ({ onExpandProject, onSelectProject }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (modalRef.current) {
        ...
        const x = (e.clientX - left - width / 2) / 20; // Adjust divisor for sensitivity
    ...
    <>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-6 auto-rows-[minmax(180px,auto)]">
            ...
            <div className="relative z-10">
                ...
                {/* Optimized Typography for Scaling */}
            ...
            {/* COMBINED ABOUT ME & SOCIALS - 2x2 (Replaces old spots) */}
            <div className="glass-panel col-span-1 md:col-span-2 row-span-2 relative overflow-hidden rounded-3xl p-6 md:p-8 group flex flex-col justify-between">
                ...
                <div className="relative z-10 flex flex-col h-full">
                    ...
                    {/* Testimonial Section */}
                    <div className="relative bg-black/5 dark:bg-white/5 rounded-xl p-4 mb-6 border border-black/5 dark:border-white/5 backdrop-blur-sm">
                        <Quote className="absolute top-2 left-2 text-primary/20 h-6 w-6 transform -scale-x-100" />
                        <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 italic text-center px-4">
                            "Working with {PORTFOLIO_OWNER} was a game-changer. His ability to blend AI with design is unmatched."
                        </p>
                        <p className="text-[10px] text-right text-primary font-medium mt-2">- Sarah J., Product Lead</p>
                    </div>
                    ...
                    <div className="mt-auto">
                        ...
                        <div className="grid grid-cols-3 gap-2 border-t border-b border-gray-200 dark:border-white/5 py-4 mb-4">
                            <div className="flex flex-col text-center md:text-left">
                                ...
                                <span className="text-sm font-semibold text-gray-900 dark:text-white">5+ Years</span>
                            </div>
                            <div className="flex flex-col text-center md:text-left">
                                <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider">Projects</span>
                                <span className="text-sm font-semibold text-gray-900 dark:text-white">20+ Ship</span>
                            </div>
                            <div className="flex flex-col text-center md:text-left">
                            ...
                            <div className="flex gap-2">
                                {SOCIALS.map((social) => (
                                    <a 
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/5 dark:bg-white/5 text-gray-600 dark:text-gray-400 transition-all hover:bg-black/10 dark:hover:bg-white/10 hover:text-black dark:hover:text-white hover:scale-110 hover:-translate-y-1 cursor-pointer"
            ...
            {/* MAP / LOCATION - 1x1 */}
            <div className="glass-panel col-span-1 row-span-1 flex flex-col items-center justify-center rounded-3xl p-6 text-center group hover:border-accent/50 transition-colors">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-blue-500/10 text-blue-500 dark:text-blue-400 group-hover:bg-blue-500/20 transition-colors">
                    <MapPin size={24} />
                </div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white transition-colors">Kerala, India</h3>
                <p className="text-sm text-gray-600 dark:text-gray-500 transition-colors">IST (GMT+5:30)</p>
                <div className="mt-4 h-1 w-12 rounded-full bg-gray-400 dark:bg-gray-700 group-hover:bg-accent transition-all duration-300 group-hover:w-20" />
            </div>
        ...
        {/* Enhanced Profile Picture Modal with Parallax */}
        {isProfileOpen && (
            <div 
            ...
            >
                <div 
                    ref={modalRef}
                    className="relative max-w-md w-full mx-4 overflow-hidden rounded-3xl border border-white/20 bg-surface/10 shadow-2xl transition-all duration-300 animate-in zoom-in-95 p-1 backdrop-blur-3xl"
                ...
                >
                    <div className="relative overflow-hidden rounded-[20px] aspect-square group">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                            <h2 className="text-2xl font-bold text-white mb-1">{PORTFOLIO_OWNER}</h2>
                    ...
                    <button 
                        onClick={() => setIsProfileOpen(false)}
                        className="absolute top-4 right-4 rounded-full bg-black/40 p-2 text-white hover:bg-white hover:text-black transition-all border border-white/10 backdrop-blur-md z-10 cursor-pointer"
                    >
                        <X size={20} />
                    </button>
                </div>
